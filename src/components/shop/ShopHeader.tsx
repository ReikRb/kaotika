...
const ShopHeader: React.FC = () => {
    ...
    return (
        <>
            <div className=" flex items-center bg-black justify-center py-2">
                <Image src="/images/HeaderDivider.webp" alt="HeaderDivider" width={350} height={1} />
            </div>
            <header className="fixed w-full bg-black text-white z-50 shadow-md">
                <div className="container mx-auto flex items-center justify-between">
                    <div className="flex items-center">
                        <button
                            onClick={() => handleActionChange('buy')}
                            className={`text-3xl text-darkSepiapx-1 py-2 hover:underline ${activeAction === 'buy' ? 'underline' : ''}`}
                        >
                            Buy
                        </button>
                        <button
                            onClick={() => handleActionChange('sell')}
                            className={`text-3xl px-1 py-2 mx-6 hover:underline ${activeAction === 'sell' ? 'underline' : ''}`}
                        >
                            Sell
                        </button>
                    </div>
                        ...
                        <nav className="flex items-center space-x-6 ml-6">
                            {categoriesToDisplay.map((category) => (
                                <button
                                    ...
                                    className={`text-3xl px-1 py-2 hover:underline ${activeCategory === category.key ? 'underline' : ''}`}
                                >
                                    {category.label}
                                </button>
                            ))}
                    ...
                    <div className="flex items-center">
                        <button
                            ...
                            className={`flex items-center text-3xl px-1 py-2 hover:underline ${activeCategory === 'cart' ? 'underline' : ''}`}
                        >
